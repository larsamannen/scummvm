...
bool iOSGraphics3dManager::gameNeedsAspectRatioCorrection() const {
	...
	// this.
	return (_gameScreenWidth == 320 && _gameScreenHeight == 200) || (_gameScreenWidth == 640 && _gameScreenHeight == 400);
}
...
bool iOSGraphics3dManager::getFeatureState(OSystem::Feature f) const {
	...
	case OSystem::kFeatureAspectRatioCorrection:
		return _asprectRatioCorrection;
	default:
		return false;
	}
}
...
void iOSGraphics3dManager::setFeatureState(OSystem::Feature f, bool enable) {
	switch (f) {
		case OSystem::kFeatureFilteringMode:
			...
			break;
		case OSystem::kFeatureAspectRatioCorrection:
			_asprectRatioCorrection = enable;
			break;
		default:
			break;
	}
...
void iOSGraphics3dManager::beginGFXTransaction() {
	_gfxTransactionError = OSystem::kTransactionSuccess;
}

OSystem::TransactionError iOSGraphics3dManager::endGFXTransaction() {
	_screenChangeCount++;
	dynamic_cast<OSystem_iOS7 *>(g_system)->updateOutputSurface();
...
void iOSGraphics3dManager::setMouseCursor(const void *buf, uint w, uint h,
										  int hotspotX, int hotspotY, uint32 keycolor, bool dontScale, const Graphics::PixelFormat *format, const byte *mask) {
...
void iOSGraphics3dManager::updateCursorScaling() {
	...
	_mouseWidthScaled = _mouseSurface->getWidth();// _mouse_texture->width();
	_mouseHeightScaled = _mouseSurface->getHeight(); // _mouse_texture->height();

	// In case scaling is actually enabled we will scale the cursor according
	// to the game screen.
	uint16 w = getWidth(); //_game_texture->width();
	uint16 h = getHeight(); //_game_texture->height();

	if (!_mouseDontScale && w && h) {
		const frac_t screen_scale_factor_x = intToFrac(_gameScreenWidth) / w;
		const frac_t screen_scale_factor_y = intToFrac(_gameScreenHeight) / h;
