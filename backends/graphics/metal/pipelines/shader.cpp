...
void ShaderPipeline::drawTextureInternal(const MetalTexture &texture, const float *coordinates, const float *texcoords) {
	...
	_renderer->draw2dTexture(_activeFramebuffer->getTargetTexture(), texture.getMetalTexture(), vertices, _projectionMatrix, _viewport, (MTL::LoadAction)_loadAction, _scissorBox, _clearColor, texture.isLinearFilteringEnabled() ? MTL::SamplerMinMagFilterLinear : MTL::SamplerMinMagFilterNearest);
}
